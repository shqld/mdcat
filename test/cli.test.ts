...
const mdcat = join(import.meta.dir, "..", "dist", "mdcat.js");
const mdgit = join(import.meta.dir, "..", "dist", "mdgit.js");
...
describe.each(["bun", "node"])("%s", (runtime) => {
  ...
  test("reports when the normalized git diff has no Markdown changes", async () => {
    ...
    expect(await spawn(runtime, mdgit, ["diff", "--color=always", "--word-diff"], directory)).toEqual(empty);

    run([...commit, "-am", "source"], directory);
    expect(await spawn(runtime, mdgit, ["show", "--color=always", "HEAD"], directory)).toEqual(empty);
  });

  test("prints mdgit help when there is no command", async () => {
    const result = await spawn(runtime, mdgit, [], import.meta.dir, new Blob(["diff --git a/x.md b/x.md\n"]));
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toStartWith("Usage: mdgit diff");
  });

  test("rejects unknown mdgit commands", async () => {
    const result = await spawn(runtime, mdgit, ["--cached"], import.meta.dir);
  ...
  test("reads a piped diff with mdcat", async () => {
    ...
    expect(await spawn(runtime, mdcat, [], import.meta.dir, new Blob([diff]))).toEqual({
      exitCode: 0,
  ...
  test("fails when mdcat cannot read a file", async () => {
    const result = await spawn(runtime, mdcat, ["missing.md"], import.meta.dir);
    expect(result.exitCode).toBe(1);
...
async function spawn(
  ...
  const process = Bun.spawn([...(runtime === "node" ? [] : [runtime]), cli, ...args], { cwd, stdin, stdout: "pipe", stderr: "pipe" });
